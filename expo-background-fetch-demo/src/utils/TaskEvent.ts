...
export default class TaskEvent {
  ...
  static async create(taskId: string, isHeadless: boolean) {
    const event = new TaskEvent(taskId, isHeadless);
    ...
    AsyncStorage.getItem('events')
      ...
      .catch((error) => {
        console.error('TaskEvent.create error: ', error);
      });
    return event;
  }

  static async all(): Promise<TaskEvent[]> {
    return new Promise((resolve, reject) => {
      AsyncStorage.getItem('events')
        .then((json) => {
          ...
          resolve(
            data.map((record: any) => {
              return new TaskEvent(record.taskId, record.isHeadless, record.timestamp);
            }),
          );
        })
        .catch((error) => {
